...
// 2. Flash Alert Message Handling
$message = "";
...
// 3. Action Handler: Delete Laptop
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    $del_id = intval($_GET['id']);
    
    $stmt_img = $conn->prepare("DELETE FROM laptop_images WHERE laptop_id = ?");
    $stmt_img->bind_param("i", $del_id);
    $stmt_img->execute();
    $stmt_img->close();

    $stmt_lap = $conn->prepare("DELETE FROM laptop WHERE id = ?");
    $stmt_lap->bind_param("i", $del_id);
    $stmt_lap->execute();
    $stmt_lap->close();
...
// 4. Action Handler: Edit Laptop (POST)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_laptop'])) {
    ...
    $update_stmt = $conn->prepare("UPDATE laptop SET 
        title=?, model=?, brand_id=?, category_id=?, price=?, stock=?, 
        processor=?, ram=?, storage=?, gpu=?, display=? 
        WHERE id=?");
    $update_stmt->bind_param(
        "ssiidisssssi",
        $title, $model, $brand_id, $cat_id, $price, $stock,
        $processor, $ram, $storage, $gpu, $display, $eid
    );
    $update_stmt->execute();
    $update_stmt->close();

    // STEP 1: Process Image Deletions FIRST
    if (!empty($_POST['delete_image_ids']) && is_array($_POST['delete_image_ids'])) {
        foreach ($_POST['delete_image_ids'] as $del_img_id) {
            ...
            $img_stmt = $conn->prepare("SELECT image_url FROM laptop_images WHERE id=? AND laptop_id=?");
            $img_stmt->bind_param("ii", $del_img_id, $eid);
            $img_stmt->execute();
            ...
            if ($img_res && $img_res->num_rows > 0) {
                ...
                $del_stmt->bind_param("ii", $del_img_id, $eid);
                $del_stmt->execute();
                $del_stmt->close();
            }
            $img_stmt->close();
    ...
    // STEP 2: Calculate Remaining Slots AFTER Deletions
    $count_stmt = $conn->prepare("SELECT COUNT(*) as cnt FROM laptop_images WHERE laptop_id = ?");
    $count_stmt->bind_param("i", $eid);
    $count_stmt->execute();
    $current_img_count = (int)($count_stmt->get_result()->fetch_assoc()['cnt'] ?? 0);
    $count_stmt->close();
    ...
    if ($slots_available > 0 && !empty($_FILES['new_gallery_images']['name']) && is_array($_FILES['new_gallery_images']['name'])) {
        ...
        foreach ($_FILES['new_gallery_images']['name'] as $key => $filename) {
            ...
            $file_size = $_FILES['new_gallery_images']['size'][$key] ?? 0;
            $file_err  = $_FILES['new_gallery_images']['error'][$key] ?? 1;
            ...
            if (!empty($filename) && $file_err === 0 && is_uploaded_file($tmp_name) && $file_size <= $max_size) {
                ...
                if (in_array($ext, $allowed_exts, true)) {
                    ...
                    if ($check_img !== false && in_array($check_img['mime'], $allowed_mimes, true)) {
                        $new_name = 'laptop_' . bin2hex(random_bytes(8)) . '.' . $ext;
...
// 5. Pre-fetch Brands & Categories for Add/Edit Modals
$brands_all = [];
$tmp = $conn->query("SELECT * FROM brand ORDER BY name ASC");
if ($tmp) { while ($b = $tmp->fetch_assoc()) $brands_all[] = $b; }

$cats_all = [];
$tmp2 = $conn->query("SELECT * FROM categories ORDER BY name ASC");
if ($tmp2) { while ($c = $tmp2->fetch_assoc()) $cats_all[] = $c; }
...
if ($search !== '') {
    $whereClauses[] = "(l.title LIKE ? OR l.model LIKE ? OR b.name LIKE ? OR c.name LIKE ? OR l.processor LIKE ? OR l.ram LIKE ? OR l.storage LIKE ? OR l.gpu LIKE ?)";
    $searchTerm     = "%{$search}%";
...
$countSql  = "SELECT COUNT(*) AS total FROM laptop l LEFT JOIN brand b ON l.brand_id = b.id LEFT JOIN categories c ON l.category_id = c.id{$whereSql}";
...
$dataStmt->execute();
$laptops = $dataStmt->get_result();

function pageUrl($p, $s) {
    $query = ['page' => $p];
    if ($s !== '') $query['search'] = $s;
    return 'inventory.php?' . http_build_query($query);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    .modal-overlay { display: none; position: fixed; inset: 0; background: rgba(15,23,42,0.45); z-index: 900; backdrop-filter: blur(4px); align-items: center; justify-content: center; }
    .modal-overlay.open { display: flex; }
    .modal-box { background: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0; width: 92%; max-width: 780px; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.15); animation: slideUp 0.25s ease; }
    @keyframes slideUp { from { opacity:0; transform:translateY(20px); } to { opacity:1; transform:translateY(0); } }
    .modal-head { display: flex; justify-content: space-between; align-items: center; padding: 1.1rem 1.4rem; border-bottom: 1px solid #e2e8f0; background: #fff; border-radius: 16px 16px 0 0; }
    .modal-head h3 { font-size: 1rem; font-weight: 700; color: #0f172a; }
    .modal-close { background: none; border: none; font-size: 1.4rem; cursor: pointer; color: #64748b; line-height: 1; padding: 0 0.3rem; }
    .modal-close:hover { color: #b91c1c; }
    .modal-body { padding: 1.4rem; }
    .btn-open-modal { display: inline-flex; align-items: center; gap: 0.4rem; background: var(--accent-blue); color: #fff; border: none; padding: 0.55rem 1.1rem; border-radius: 8px; font-size: 0.88rem; font-weight: 700; cursor: pointer; transition: background 0.2s; }
    .btn-open-modal:hover { background: var(--accent-blue-hover); }
    .btn-edit { padding: 0.18rem 0.36rem; border-radius: 4px; font-size: 0.62rem; font-weight: 600; text-decoration: none; background: #dbeafe; color: #1d4ed8; border: none; cursor: pointer; display: inline-block; margin-right: 2px; white-space: nowrap; }
    .btn-edit:hover { background: #1d4ed8; color: #fff; }
    .btn-del { padding: 0.18rem 0.36rem; border-radius: 4px; font-size: 0.62rem; font-weight: 600; text-decoration: none; background: #fee2e2; color: #b91c1c; border: none; cursor: pointer; display: inline-block; white-space: nowrap; }
    ...
    #inventoryTable th:nth-child(6), #inventoryTable td:nth-child(6) { width: 12%; white-space: nowrap; font-weight: 700; }
    ...
    #inventoryTable th:nth-child(9), #inventoryTable td:nth-child(9) { width: 11%; overflow: visible; }
  ...
  <main class="main-content">
    <header class="header">
      ...
      <div style="display:flex;align-items:center;gap:0.6rem;">
        ...
        <button class="btn-open-modal" onclick="openModal('addModal')">+ Add Catalog</button>
      </div>
    </header>
    ...
    <section class="table-container">
      <div style="padding:1rem 1.5rem;border-bottom:1px solid var(--border-color);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:15px;">
        <h3>Laptop Catalog</h3>

        <form method="GET" action="inventory.php" style="display:flex;gap:10px;align-items:center;">
          <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search by title, model, brand, category..." style="width:300px;padding:10px 14px;border:1px solid #d1d5db;border-radius:8px;font-size:14px;">
          <button type="submit" style="padding:10px 18px;background:#2563eb;color:white;border:none;border-radius:8px;cursor:pointer;font-weight:600;">Search</button>
          <?php if($search !== ''): ?>
            <a href="inventory.php" style="padding:10px 18px;background:#ef4444;color:white;border-radius:8px;text-decoration:none;font-weight:600;">Clear</a>
          <?php endif; ?>
        </form>
      </div>
          ...
          <?php if ($laptops && $laptops->num_rows > 0): ?>
            <?php while ($row = $laptops->fetch_assoc()): ?>
              <?php
                ...
                $display_img = filter_var($img_val, FILTER_VALIDATE_URL) ? $img_val : (!empty($img_val) ? '../uploads/' . $img_val : '');
                $proc = $row['processor'] ?? '';
                $ram  = $row['ram']  ?? '';
                $stor = $row['storage'] ?? '';
              ...
              <tr>
                ...
                <td style="white-space:nowrap;">
                  ...
                  <a href="inventory.php?action=delete&id=<?php echo $row['id']; ?>" class="btn-del" onclick="return confirm('Delete this laptop from catalog?')">Delete</a>
                </td>
              </tr>
            <?php endwhile; ?>
          <?php else: ?>
            <tr>
              <td colspan="9" style="text-align:center;color:var(--text-muted);padding:2rem;">No laptops in catalog matching your criteria.</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
      ...
      <div style="padding: 1.5rem; display: flex; justify-content: center; gap: 0.5rem; align-items: center;">
        <?php if ($page > 1): ?>
          <a href="<?php echo pageUrl($page - 1, $search); ?>" style="padding: 0.5rem 1rem; border: 1px solid var(--border-color); border-radius: 8px; text-decoration: none; color: var(--text-dark); font-weight: 600;">&laquo; Prev</a>
        <?php endif; ?>
        
        <span style="font-size: 0.9rem; font-weight: 600; color: var(--text-muted);">Page <?php echo $page; ?> of <?php echo $total_pages; ?></span>

        <?php if ($page < $total_pages): ?>
          <a href="<?php echo pageUrl($page + 1, $search); ?>" style="padding: 0.5rem 1rem; border: 1px solid var(--border-color); border-radius: 8px; text-decoration: none; color: var(--text-dark); font-weight: 600;">Next &raquo;</a>
        <?php endif; ?>
      </div>
      <?php endif; ?>
      ...
      <div class="modal-body">
        <form action="inventory.php" method="POST" enctype="multipart/form-data" id="editForm">
          ...
          <div style="margin:1.2rem 0 0.6rem;">
            ...
            <div style="font-size:0.72rem;color:#64748b;margin-bottom:0.7rem;">Each laptop can have up to <strong>4 images</strong>. Click the red 🗑 button to remove an image when saving changes.</div>
...
<script>
  ...
  function openEditModal(row) {
    ...
    document.querySelectorAll('.staged-del-input').forEach(el => el.remove());
  ...
  function deleteImage(imgId) {
    ...
    hiddenInput.className = 'staged-del-input';
    ...
    if (currentImageCount === 0) {
      document.getElementById('editImageGallery').innerHTML = '<span style="color:#94a3b8;font-size:0.8rem;">No images remaining — upload replacements below.</span>';
    }
  }
